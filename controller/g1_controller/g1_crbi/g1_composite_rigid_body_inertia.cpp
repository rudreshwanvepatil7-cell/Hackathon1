...
G1CompositeRigidBodyInertia::G1CompositeRigidBodyInertia()
    : CompositeRigidBodyInertia(g1_crbi_helper_n_in(), 3) {
  util::PrettyConstructor(2, "G1CompositeRigidBodyInertia");

  g1_crbi_helper_work(&f_sz_arg_, &f_sz_res_, &f_sz_iw_, &f_sz_w_);
...
Eigen::Matrix3d G1CompositeRigidBodyInertia::ComputeInertia(
  ...
  double f_w[f_sz_w_];
  g1_crbi_helper(const_cast<const double **>(f_x_), f_y_, f_iw, f_w,
                     g1_crbi_helper_checkout());
...
// Eigen::MatrixXd G1CompositeRigidBodyInertia::ComputeDerivativeWrtInput(
