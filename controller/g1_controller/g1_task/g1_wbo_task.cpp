...
G1WBOTask::G1WBOTask(PinocchioRobotSystem *robot) : Task(robot, 1) {

  util::PrettyConstructor(3, "G1WBOTask");
  ...
  actuated_joint_idx_ = {g1_joint::l_hip_ie,      g1_joint::l_hip_aa,
                         g1_joint::l_hip_fe,      g1_joint::l_knee_fe_jp,
                         g1_joint::l_knee_fe_jd,  g1_joint::l_shoulder_fe,
                         g1_joint::l_shoulder_aa, g1_joint::l_shoulder_ie,
                         g1_joint::l_elbow_fe,    g1_joint::r_hip_ie,
                         g1_joint::r_hip_aa,      g1_joint::r_hip_fe,
                         g1_joint::r_knee_fe_jp,  g1_joint::r_knee_fe_jd,
                         g1_joint::r_shoulder_fe, g1_joint::r_shoulder_aa,
                         g1_joint::r_shoulder_ie, g1_joint::r_elbow_fe};

  ignored_joint_idx_ = {g1_joint::l_hip_ie,     g1_joint::l_hip_aa,
                        g1_joint::l_hip_fe,     g1_joint::l_knee_fe_jp,
                        g1_joint::l_knee_fe_jd, g1_joint::r_hip_ie,
                        g1_joint::r_hip_aa,     g1_joint::r_hip_fe,
                        g1_joint::r_knee_fe_jp, g1_joint::r_knee_fe_jd};
...
/*
// full joint space
void G1WBOTask::UpdateOpCommand(const Eigen::Matrix3d &rot_world_local) {
...
void G1WBOTask::UpdateOpCommand(const Eigen::Matrix3d &world_R_local) {
...
void G1WBOTask::UpdateJacobianDotQdot() {
  jacobian_dot_q_dot_ = Eigen::VectorXd::Zero(dim_);
}

Eigen::Vector3d
G1WBOTask::_LocalWboQuatXYZ(const Eigen::VectorXd &joint_pos) {
...
Eigen::MatrixXd
G1WBOTask::_WboQuatXYZJacobian(const Eigen::VectorXd &joint_pos) {
...
void G1WBOTask::_CopyEigenMatrixToDoubleArray(const Eigen::MatrixXd &mat,
                                                 double **array_2d) {
...
void G1WBOTask::_CopyEigenMatrixToDoubleArray(const Eigen::MatrixXd &mat1,
...
void G1WBOTask::_CopyEigenVectorToDoubleArray(const Eigen::VectorXd &vec,
                                                 double **array_2d) {
  for (int i = 0; i < vec.size(); i++) {
    array_2d[0][i] = vec[i];
  }
}
void G1WBOTask::_CopyDoubleArrayToEigenMatrix(double **array_2d,
                                                 Eigen::MatrixXd &mat,
                                                 const int block_row,
                                                 const int block_col) {
...
G1WBOTask::_QuatRateToAngVel(const Eigen::Quaterniond &quat) {
  double x = quat.x();
  double y = quat.y();
  double z = quat.z();
  double w = quat.w();
...
G1WBOTask::_QuatMiscFunc(const Eigen::Quaterniond &quat) {
...
Eigen::VectorXd G1WBOTask::_GetJointPosReduced() {
...
Eigen::VectorXd G1WBOTask::_GetJointVelReduced() {
