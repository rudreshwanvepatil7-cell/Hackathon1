...
void MPCLocomotion_WBIC::FirstVisit() {
  std::cout << "g1_states_wbic::kMPCLocomotion" << std::endl;
  ...
  for (int foot = 0; foot < foot_side::NumFoot; foot++) {
    if (mpc_interface->contact_state_[foot] > 0.0) // in contact
    {
      if (foot == foot_side::LFoot) {
        ...
        // robot_->GetLinkIsometry(g1_link::l_foot_contact).linear());
        ...
        // robot_->GetLinkIsometry(g1_link::r_foot_contact).linear());
        //
        // TEST
        if (prev_contact_states_[foot] == 0.0) {
