...
CrabCoMZTask::CrabCoMZTask(PinocchioRobotSystem *robot) : Task(robot, 1) {

  util::PrettyConstructor(3, "CrabCoMZTask");
  sp_ = CrabStateProvider::GetStateProvider();
}

void CrabCoMZTask::UpdateOpCommand(const Eigen::Matrix3d &world_R_local) {
  if (com_height_ == com_height::kCoM) {
    pos_ << robot_->GetRobotComPos()[2];
    vel_ << sp_->com_vel_est_[2];

  } else if (com_height_ == com_height::kBase) {
    pos_ << robot_->GetLinkIsometry(crab_link::base_link).translation()[2];
    vel_ << robot_->GetLinkSpatialVel(crab_link::base_link)[5];
  }
...
void CrabCoMZTask::UpdateJacobian() {
  if (com_height_ == com_height::kCoM) {
    jacobian_ = robot_->GetComLinJacobian().bottomRows<1>();
  } else if (com_height_ == com_height::kBase) {
    jacobian_ = robot_->GetLinkJacobian(crab_link::base_link).bottomRows<1>();
  }
}

void CrabCoMZTask::UpdateJacobianDotQdot() {
  if (com_height_ == com_height::kCoM) {
    jacobian_dot_q_dot_ << robot_->GetComLinJacobianDotQdot()[2];
  } else if (com_height_ == com_height::kBase) {
    jacobian_dot_q_dot_ << robot_->GetLinkJacobianDotQdot(
        crab_link::base_link)[5];
  }
}

void CrabCoMZTask::SetParameters(const YAML::Node &node,
