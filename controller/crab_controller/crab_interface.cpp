...
CrabInterface::CrabInterface() : Interface() {
  std::string border = "=";
  for (unsigned int i = 0; i < 79; ++i) border += "=";
...
void CrabInterface::GetCommand(void *sensor_data, void *command_data) {
  ...
  CrabSensorData *draco_sensor_data =
      static_cast<CrabSensorData *>(sensor_data);
...
void CrabInterface::_SetParameters() {
  ...
  // if (!CrabDataManager::GetDataManager()->IsInitialized()) {
